...
def optimize_file(path, bit_rate, mode, frequency):
    ...
    try:
        ...
        # Extract the ID3 tags from the original file
        ...
        qscale = scale_qality(bit_rate)
        tags = {key: value for key, value in tags.items() if key != ''}
        
        audio.export(output_file, format= "mp3", bitrate= bit_rate, parameters= ["-c:a", "libmp3lame", "-ar", str(frequency)] + qscale.split(), tags= tags)
