...
_FIXES = (
    ...
    (
        "This FFmpeg build",
        "download the 'essentials' build from gyan.dev and copy ffmpeg.exe + ffprobe.exe into src.",
    ),
    (
    ...
    ("The input audio reports", "this file looks damaged. Try another copy of the song."),
    (
    ...
    ("Output parent is not a directory", "save into a normal folder, like your Music folder."),
    ("Output path exists but is not a regular file", "a folder has that name. Pick another name."),
    (
    ...
    (
        "rotation_seconds",
        f"use a number from 2 to 100, e.g. --rotation-seconds {_BEST.rotation_seconds:g} (best).",
    ),
    ("intensity", f"use a number from 0 to 1, e.g. --intensity {_BEST.intensity:.2f} (best)."),
    ("ambience", f"use a number from 0 to 1, e.g. --ambience {_BEST.ambience:.2f} (best)."),
    (
        "limiter_ceiling",
        f"use 0.0625 to 1, e.g. --limiter-ceiling {_BEST.limiter_ceiling:.2f} (best).",
    ),
    ("mp3_bitrate", "use 128, 160, 192, 224, 256 or 320 kbps, e.g. --bitrate 320 (best)."),
    (
    ...
    ("mp3_quality", f"use a whole number from 0 to 9, e.g. --quality {_BEST.mp3_quality} (best)."),
    ("loudness_target", "use a number from -30 to -5, e.g. --loudness -14 (best), or 'off'."),
    ("FFmpeg conversion failed", "try again with --verbose added to see more details."),
    ("FFmpeg reported success", "try again; if it keeps happening, try with --verbose."),
    (
    ...
    ("Unable to publish", "check the disk has free space and the drive is still plugged in."),
...
def fix_for(error: Audio8DError) -> str | None:
    ...
    if isinstance(error, DependencyError) and message.startswith(
        ...
        return "FFmpeg would not run. Download the 'essentials' build from gyan.dev into src."
    if message.startswith("Failed to start"):
        return "FFmpeg would not start. Put fresh copies of ffmpeg.exe and ffprobe.exe in src."
...
def usage_fix_for(message: str) -> str:
    ...
    if "--preset" in message:
        return f"use one of: {', '.join(PRESETS)}. Best: --preset {RECOMMENDED_PRESET}"
    if "--loudness" in message:
        return "write a number such as -14 (best), or the word off."
    if "unrecognized arguments" in message:
        return 'one word was not understood. Put song names with spaces inside "quotes".'
    if "expected one argument" in message:
        return "that option needs a value after it, e.g. --intensity 0.8"
    if "invalid float value" in message:
        return "that option needs a number, e.g. --intensity 0.8"
